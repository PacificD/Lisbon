...
import { draftRenderer, renderNewsletterContent } from '../../packages/email/src/render.ts'

describe('newsletter email rendering', () => {
  it('renders stable HTML and text for previews and sends', async () => {
    const rendered = await renderNewsletterContent({
      theme: {
        id: 'theme-frontend',
        slug: 'frontend',
        name: 'Frontend',
